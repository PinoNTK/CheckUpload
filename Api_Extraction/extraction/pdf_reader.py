...
class PDFObject(object):
    ...
    def __init__(self,type, x0, y0,x1,y1,text=None,font_style = None,font_size = None,font_name = None):
...
def parse_textline(textline):
    ...
    return PDFObject('TextLine',x0,y0,x1,y1,text=text,font_name=font,font_size=size,font_style=style) #[text, font, style, size, x0, x1, y0, y1]
...
def is_newsentence(textline,before):
    ...
    if (textline.text.strip()[0] in ['+','-','*']) and before.x1>textline.x0:
        return True
    if before.x1>textline.x0 and textline.is_title:
        return True
    if before.is_title:
        return True
    return False
